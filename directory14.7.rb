...
def create_new_file
  ...
  if new_file == STUDENT_DATABASE
    save_to_current_file
  else
    puts "Error, please save as students.csv"
    create_new_file
  end
end
...
def save_to_current_file
  ...
  CSV.open(STUDENT_DATABASE, "w") do |f| # the filename is loaded and closed automatically when done by passing load in a block
    # iterate over the array of students
    puts "Filename: #{STUDENT_DATABASE} saved."
...
def load_from_file(filename = STUDENT_DATABASE)
    CSV.open(filename, "r") do |f| # the filename is loaded and closed automatically when done by passing load in a block
