...
def save_to_current_file
  ...
  File.open(STUDENT_DATABASE, "w") do |file| # the filename is loaded and closed automatically when done by passing load in a block
...
def load_from_file(filename = STUDENT_DATABASE)
    File.open(filename, "r") do |f| # the filename is loaded and closed automatically when done by passing load in a block
